...
describe('Asset Transfer Besic REST API', () => {
  ...
  describe('/api/assets', () => {
    let mockGetAllPollsTransaction: MockProxy<Transaction>;

    beforeEach(() => {
      mockGetAllPollsTransaction = mock<Transaction>();
      const mockBasicContract = mock<Contract>();
      mockBasicContract.createTransaction
        .calledWith('GetAllPolls')
        .mockReturnValue(mockGetAllPollsTransaction);
      app.locals[config.mspIdOrg1] = {
    ...
    it('GET should respond with an empty json array when there are no assets', async () => {
      mockGetAllPollsTransaction.evaluate.mockResolvedValue(Buffer.from(''));
    ...
    it('GET should respond with json array of assets', async () => {
      mockGetAllPollsTransaction.evaluate.mockResolvedValue(
        mockAllAssetsBuffer
      );
